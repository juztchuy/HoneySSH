...
class FrontendSSHTransport(transport.SSHServerTransport, TimeoutMixin):
    ...
    def __init__(self):
        ...
        # Ollama LLM client for processing SSH commands
    ...
    def packet_buffer(self, messageNum: int, payload: bytes) -> None:
        ...
        to server in here. Also forward SSH data to the Ollama LLM client for analysis.
        """
        if not self.backendConnected:
            ...
            if len(self.delayedPackets) > 0:
                ...
                # Send data to Ollama client for LLM processing
                try:
                    self.ollama_client.process_ssh_data(
                        session_id=self.transportId,
                        message_num=messageNum,
                        payload=payload,
                        user=self.sshParse.client.user if self.sshParse.client else None,
                    )
                except Exception as e:
                    log.err(f"Error sending data to Ollama client: {e}")
